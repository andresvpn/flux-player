...
class Player {
  ...
  _addSeekButtons() {
    ...
    // Botón de retroceso (ahora primero)
    this._playerInstance.addButton(
    ...
    // Botón de avance (ahora después del retroceso)
    this._playerInstance.addButton(
  ...
  const controlss = document.querySelector('.jw-button-container');
    if (controlss) {
        const buttonFF11 = controls.querySelector('[button="ff11"]');
        const buttonFF00 = controls.querySelector('[button="ff00"]');
        const volumeButton = controls.querySelector('.jw-icon-volume');

        if (buttonFF11 && buttonFF00 && volumeButton) {
